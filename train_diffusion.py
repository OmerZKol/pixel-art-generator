...
class PixelArtDataset(Dataset):
    def __init__(self, image_dir, transform=None):
        ...
        self.object_types = ['rectangle', 'circle', 'line', 'face']  # 4 types
        self.colors = ['red', 'green', 'blue', 'yellow', 'magenta', 'cyan', 'orange', 'purple']  # 8 colours
    ...
    def __getitem__(self, idx):
        ...
        shape = split[1]
        colour = split[2]
        shape_idx = self.object_types.index(shape)
        colour_idx = self.colour.index(colour)
...
class UNet(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, time_emb_dim=128, num_shape_types=4, num_colours=8):
        ...
        # Decoder (upsampling)
        self.up1 = ResidualBlock(128 + 128, 64, time_emb_dim)  # Skip connection
        self.up2 = ResidualBlock(64 + 64, 32, time_emb_dim)   # Skip connection
...
class PixelArtDiffusion:
    ...
    @torch.no_grad()
    def sample(self, num_samples=16, guidance_scale=1.0):
        ...
        for t in reversed(range(self.num_timesteps)):
            ...
            # Predict noise
            noise_pred = self.model(img, timestep)
...
def main():
    save_directory = "samples_diffusion/no_labels"
    ...
    diffusion = PixelArtDiffusion(device=device, num_timesteps=100)
    ...
    diffusion.save_samples(epoch, save_dir="samples_diffusion")
